...
describe("VN devtools view-model helpers", () => {
  ...
  it("maps absolute UTF-16 spans through CRLF and astral characters", () => {
    const sourceText = "😀\r\n@bgm volume:fast";
    expect(resolveVnDevtoolsLineRange(sourceText, 1, { start: 0, end: 2 }))
      .toEqual({ start: 0, end: 2 });
    expect(resolveVnDevtoolsLineRange(sourceText, 2, { start: 16, end: 20 }))
      .toEqual({ start: 12, end: 16 });
    expect(resolveVnDevtoolsLineRange(sourceText, 1, { start: 16, end: 20 })).toBeUndefined();
    expect(resolveVnDevtoolsLineRange(sourceText, 2, { start: 2, end: 4 })).toBeUndefined();
    expect(resolveVnDevtoolsLineRange(sourceText, 3, { start: 16, end: 20 })).toBeUndefined();
  });
