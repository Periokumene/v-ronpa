...
function createLineModel(
  ...
  for (const diagnostic of diagnostics) {
    ...
    diagnosticsByLine.set(diagnostic.lineId, [...(diagnosticsByLine.get(diagnostic.lineId) ?? []), diagnostic]);
  }
...
function createDiagnostics(
  inspection: VnDebugEntryInspection | undefined,
  runtimeDiagnostics: readonly { code: string; severity: "info" | "warning" | "error"; message: string; loc?: string }[],
  ...
  const runtime = [...(inspection?.diagnostics ?? []), ...runtimeDiagnostics].map((diagnostic, index) => {
    const lineNumber = diagnostic.loc ? Number(diagnostic.loc.match(/:(\d+):\d+$/u)?.[1]) : undefined;
    return {
