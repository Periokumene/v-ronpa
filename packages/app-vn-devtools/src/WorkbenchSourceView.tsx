...
import {
  ...
  type VnDevtoolsDiagnosticSeverity,
      ...
      >
        ...
        <code>
          {renderSourceText(
            line.sourceText,
            match?.sourceRanges ?? [],
            (line.diagnostics ?? []).flatMap((diagnostic) => diagnostic.sourceRange
              ? [{ range: diagnostic.sourceRange, severity: diagnostic.severity }]
              : [])
          )}
        </code>
      </button>
...
interface SourceDiagnosticRange {
  range: VnDevtoolsSourceRange;
  severity: VnDevtoolsDiagnosticSeverity;
}

function renderSourceText(
  sourceText: string,
  ranges: readonly VnDevtoolsSourceRange[],
  diagnosticRanges: readonly SourceDiagnosticRange[]
): ReactNode {
  ...
  for (const { range } of diagnosticRanges) {
    if (range.start < 0 || range.end <= range.start || range.end > sourceText.length) continue;
    boundaries.add(range.start);
    boundaries.add(range.end);
  }
  ...
  return ordered.slice(0, -1).map((start, index) => {
    ...
    const diagnosticSeverity = highestDiagnosticSeverity(
      diagnosticRanges
    ...
    const className = [
      `syntax-${kind}`,
      diagnosticSeverity && "has-diagnostic",
    ...
    return matched
      ? (
          <mark
            key={`${start}:${end}`}
            className={className}
            data-diagnostic-severity={diagnosticSeverity}
          >
            {text}
          </mark>
        )
...
function highestDiagnosticSeverity(
  severities: readonly VnDevtoolsDiagnosticSeverity[]
): VnDevtoolsDiagnosticSeverity | undefined {
