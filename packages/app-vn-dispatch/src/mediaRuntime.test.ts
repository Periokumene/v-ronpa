...
describe("media runtime", () => {
  it("tracks BGM by group and crossfades only the matching group", () => {
    ...
    expect(first.state.bgmByGroup).toEqual({
      music: { assetId: "bgm/alt", volume: 0.7 },
      ambient: { assetId: "bgm/layer", volume: 0.25 }
    });
    expect(first.effects).toEqual([
      { type: "play-bgm", key: "music", group: "music", assetId: "bgm/main", volume: 0.45, fadeInMs: 300 },
      { type: "play-bgm", key: "ambient", group: "ambient", assetId: "bgm/layer", volume: 0.25 },
      { type: "stop-bgm", key: "music", group: "music", fadeMs: 500 },
      { type: "play-bgm", key: "music", group: "music", assetId: "bgm/alt", volume: 0.7, fadeInMs: 500 }
    ]);
  });
  ...
  it("adjusts active BGM volume by group without restarting playback", () => {
    ...
    expect(sameTrack.state.bgmByGroup.music).toEqual({ assetId: "bgm/main", volume: 0.2 });
  ...
  it("tracks only looping SFX and clears them through stopSfx", () => {
    ...
    expect(started.state.loopingSfxByKey).toEqual({
      rain: { group: "rain", assetId: "sfx/rain", volume: 0.35 }
    });
    expect(started.effects).toEqual([
      { type: "play-sfx", assetId: "sfx/rain", loop: true, fast: false, key: "rain", group: "rain", volume: 0.35, fadeInMs: 500 },
      { type: "play-sfx", assetId: "sfx/door", loop: false, fast: false, volume: 0.9, fadeInMs: 250 },
      { type: "play-sfx", assetId: "sfx/shock", loop: false, fast: true, volume: 0.75 }
    ]);
  ...
  it("adjusts active loop SFX volume by key without restarting playback", () => {
    ...
    expect(byGroup.state.loopingSfxByKey.rain).toEqual({ group: "rain", assetId: "sfx/rain", volume: 0.15 });
    ...
    expect(sameLoop.state.loopingSfxByKey.rain).toEqual({ group: "rain", assetId: "sfx/rain", volume: 0.2 });
  ...
  it("uses sfxPath as the loop key when group is absent and diagnoses missing stop keys", () => {
    ...
    expect(started.state.loopingSfxByKey).toEqual({
      "sfx/hum": { assetId: "sfx/hum", volume: 1 }
    });
  ...
  it("emits movie playback descriptors without storing movie handles", () => {
    ...
    expect(result.state).toEqual(createInitialMediaRuntimeState());
    expect(result.effects).toEqual([
      { type: "play-movie", assetId: "video/validation-intro", block: true, durationMs: 1200 }
    ]);
    expect(result.diagnostics).toEqual([]);
  });
  ...
  it("round-trips canonical persistent media and materializes deterministic restore effects", () => {
    const state: MediaRuntimeState = {
      bgmByGroup: {
        ...
        ambient: { assetId: "bgm/layer", volume: 0.2 }
      },
      loopingSfxByKey: {
    ...
    expect(createVnMediaRestoreEffects(checkpoint)).toEqual([
      { type: "play-bgm", key: "ambient", group: "ambient", assetId: "bgm/layer", volume: 0.2 },
      ...
      { type: "play-sfx", key: "sfx/hum", assetId: "sfx/hum", loop: true, fast: false, volume: 0.8 }
    ]);
  });
});
