...
function generateRuntimeAssetsModule(assets, fontFaces, config) {
  ...
  validateScriptCatalog(scriptArtifacts, config);
...
function generateScriptMetadataModule(entries, exportName) {
  const artifacts = collectScriptArtifacts(entries, gameAAssetConfig);
  ...
  return [
    ...
    "",
    `export const ${gameAAssetConfig.testScriptSourcesExportName} = ${scriptSourcesBody} satisfies Record<string, VnRuntimeScriptSource>;`,
    ""
  ].join("\n");
}
...
function validateScriptCatalog(artifacts, config) {
  if (!config.entryInitialScriptPath) return;
  const linked = linkRuntimeScriptCatalog(
    {
      initialScriptPath: config.entryInitialScriptPath,
      ...(config.entryStartLabel ? { startLabel: config.entryStartLabel } : {})
    },
    artifacts.map((artifact) => artifact.compiledScript)
  );
  if (linked.diagnostics.length > 0) {
    throw new Error(`${config.id} script catalog failed linking: ${linked.diagnostics.map((diagnostic) => diagnostic.message).join("; ")}`);
  }
}
