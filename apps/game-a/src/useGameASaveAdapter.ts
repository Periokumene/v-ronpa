...
export const GAME_A_SAVE_DB_NAME = "v-ronpa-game-a-saves-v13";
...
export interface GameASaveAdapterOptions {
  ...
  onLoad: (save: SaveData) => Promise<{ ok: true } | { ok: false; code: string; message: string } | void> | { ok: true } | { ok: false; code: string; message: string } | void;
