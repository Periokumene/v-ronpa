...
export type HarnessShowcaseDiagnosticSource = "parser" | "compiler" | "story" | "transaction" | "media" | "ui" | "trial" | "asset";

export interface HarnessShowcaseRuntimeDiagnostic {
...
export interface HarnessShowcaseRuntimeRestorePlan {
  diagnostics: HarnessShowcaseRuntimeDiagnostic[];
...
export interface MediaSourceResolverResult {
  uri?: string;
  diagnostic?: HarnessShowcaseRuntimeDiagnostic;
}
...
export interface HarnessShowcasePresentationTransactionInput {
...
export function useHarnessShowcaseRuntimeAdapter(
  ...
  const parsed = useMemo(
    () => parseScenario({ sourceText: harnessShowcaseScript, scriptPath: "harness/harness-showcase.nani" }),
    []
  );
  ...
  const initialRuntimeDiagnostics = useMemo(
    () => createInitialHarnessShowcaseDiagnostics(parsed.diagnostics, compiled.diagnostics, trialDefinitionDiagnostics),
  ...
  const [runtimeDiagnostics, setRuntimeDiagnostics] = useState<HarnessShowcaseRuntimeDiagnostic[]>(() => initialRuntimeDiagnostics);
  ...
  const appendRuntimeDiagnostics = useCallback((diagnostics: HarnessShowcaseRuntimeDiagnostic[]) => {
  ...
  const observeAssetDiagnostic = useCallback(
    (diagnostic: { code?: string; severity?: "info" | "warning" | "error"; message: string; assetId?: string; kind?: string }) => {
      appendRuntimeDiagnostics([toHarnessShowcaseAssetDiagnostic(diagnostic)]);
    },
  ...
  function submitStoryInput(value: string | number | boolean) {
    ...
    appendRuntimeDiagnostics(collectHarnessShowcaseRuntimeDiagnostics({ storyDiagnostics: submitted.diagnostics }));
  ...
  function restoreFromSave(save: Pick<SaveData, "mode" | "navi" | "story" | "pixiStage" | "inventory" | "evidence" | "characters" | "trial">) {
    ...
    observedWaitTasksRef.current = undefined;
    completingWaitKeyRef.current = undefined;
    const plan = createHarnessShowcaseRuntimeRestorePlan(save, compiled.script);
  ...
  function commitStoryTransaction({
    ...
    const transaction = createHarnessShowcasePresentationTransaction({
    ...
    const movieDiagnostics: HarnessShowcaseRuntimeDiagnostic[] = [];
    ...
    appendRuntimeDiagnostics(
      collectHarnessShowcaseRuntimeDiagnostics({
  ...
  function completeRuntimeWaitAndAdvance(source: StoryPlayAdvanceSource, kind: "pause" | "movie") {
    ...
    appendRuntimeDiagnostics(collectHarnessShowcaseRuntimeDiagnostics({ storyDiagnostics: completed.diagnostics }));
  ...
  const interactionContext: GameInteractionContext = useMemo(
    () => createHarnessShowcaseInteractionContext({ flowMode, navi, storyRuntime, trialRuntime }),
    [
      flowMode,
      navi.inputLock,
      navi.substate,
...
export function createInitialHarnessShowcaseDiagnostics(
...
): HarnessShowcaseRuntimeDiagnostic[] {
  return limitRuntimeDiagnostics([
    ...parserDiagnostics.map(toHarnessShowcaseParserDiagnostic),
    ...compilerDiagnostics.map(toHarnessShowcaseCompilerDiagnostic),
    ...trialDiagnostics.map(toHarnessShowcaseTrialDiagnostic)
  ]);
}

export function collectHarnessShowcaseRuntimeDiagnostics({
  mediaDiagnostics = [],
  storyDiagnostics = [],
  ...
  return [
    ...storyDiagnostics.map(toHarnessShowcaseStoryDiagnostic),
    ...transactionDiagnostics.map(toHarnessShowcaseTransactionDiagnostic),
    ...mediaDiagnostics.map(toHarnessShowcaseMediaDiagnostic),
    ...uiDiagnostics.map(toHarnessShowcaseUiDiagnostic)
  ];
}
...
export function resolveDialogueVoiceAssetAvailability({
  ...
  if (resolved.diagnostic?.code === "asset-kind-mismatch") {
    return {
      available: false,
      diagnostics: [toHarnessShowcaseAssetDiagnostic({ ...resolved.diagnostic, severity: "warning" })],
      sourceRef
    };
  }
...
export function createHarnessShowcasePresentationTransaction({
  runtimeCommands,
  previousMediaState,
  previousPixiStage,
  previousUiState,
  options = {}
}: HarnessShowcasePresentationTransactionInput): VnRuntimePresentationTransaction {
...
export function resolveMediaSource({
  ...
  return {
    diagnostic: toHarnessShowcaseAssetDiagnostic(
      resolved.diagnostic ?? {
        code: "asset-missing",
        severity: "error",
        id: sourceRef,
...
export async function applyMediaRuntimeEffects({
  ...
  const diagnostics: HarnessShowcaseRuntimeDiagnostic[] = [];
...
function mediaHandleMissing(message: string): HarnessShowcaseRuntimeDiagnostic {
  return { source: "media", code: "media-handle-missing", severity: "info", message };
}

function mediaPortError(message: string): HarnessShowcaseRuntimeDiagnostic {
...
function toHarnessShowcaseParserDiagnostic(diagnostic: ParserDiagnostic): HarnessShowcaseRuntimeDiagnostic {
  return {
    source: "parser",
...
function toHarnessShowcaseCompilerDiagnostic(diagnostic: RuntimeCompilerDiagnostic): HarnessShowcaseRuntimeDiagnostic {
  return {
    source: "compiler",
...
function toHarnessShowcaseStoryDiagnostic(diagnostic: StoryStepperDiagnostic): HarnessShowcaseRuntimeDiagnostic {
  return {
    source: "story",
    code: diagnostic.code,
    severity: diagnostic.severity ?? "warning",
...
function toHarnessShowcaseTransactionDiagnostic(
...
function toHarnessShowcaseMediaDiagnostic(diagnostic: MediaRuntimeDiagnostic): HarnessShowcaseRuntimeDiagnostic {
  return {
    source: "media",
    code: diagnostic.code,
    severity: diagnostic.severity,
...
function toHarnessShowcaseUiDiagnostic(diagnostic: UiRuntimeDiagnostic): HarnessShowcaseRuntimeDiagnostic {
  return {
    source: "ui",
    code: diagnostic.code,
    severity: diagnostic.severity,
...
function toHarnessShowcaseAssetDiagnostic(
...
function limitRuntimeDiagnostics(diagnostics: HarnessShowcaseRuntimeDiagnostic[]): HarnessShowcaseRuntimeDiagnostic[] {
