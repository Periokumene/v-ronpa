...
import {
  ...
  createInitialHarnessShowcaseDiagnostics,
...
describe("harness showcase runtime adapter helpers", () => {
  it("keeps the harness-showcase script as a Pixi command showcase without parser or compiler diagnostics", () => {
  ...
  it("keeps the harness-showcase rich text showcase covering the full first-pass markup set", () => {
    for (const sample of [
      "<b>",
      "<strong>",
      "<i>",
  ...
  it("extracts a small GameInteractionContext from harness showcase runtime state", () => {
  ...
  it("passes harness-showcase route options into the unified Story/Pixi transaction", () => {
  ...
  it("normalizes parser and compiler diagnostics for the harness readout", () => {
    ...
    expect(createInitialHarnessShowcaseDiagnostics(parsed.diagnostics, compiled.diagnostics)).toEqual([
  ...
  it("plans restore of Navi, Story, and Gameplay without carrying transient UI state", () => {
    ...
    const plan = createHarnessShowcaseRuntimeRestorePlan(save, runtimeScript);
