...
- `apps/game-a`: VN-first framework app for continued visual novel development. It owns a separate content manifest, inline VN script source, and `/game-a/**` public assets; it mounts VN shell/Pixi adapter surfaces through `AssetRegistry` runtime asset ids only, uses `media-save` for unified save slots, and does not connect Navi, Trial, or R3F.
