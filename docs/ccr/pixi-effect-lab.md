...
Add nine semantic Nani effect commands: transient `impact`, `afterimage`, `shutter`, and `flicker`; persistent `vignette`, `staticFilter`, `waterVeil`, `signalMask`, and `pulse`.
...
- Add semantic command definitions and strict normalizers. Time is authored in seconds and normalized to `durationMs`; random commands carry an explicit seed.
