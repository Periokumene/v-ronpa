...
- Source visual truth: accepted source-first IDE direction recorded with the
  original Workbench design task; repository evidence below is the canonical
...
- Focused implementation states:
  ...
  - `test-results/game-a-workbench-problems-720.png`
  - `test-results/game-a-workbench-problems-420.png`
  - `test-results/game-a-workbench-problems-320.png`
  - `test-results/game-a-workbench-problems-overlay.png`
...
- Exact compiler token underlining through UTF-16 spans, including an overlapping
  Find mark, at 720px, default 420px, 320px, and the narrow-screen overlay.
...
## Convergence exact-diagnostic review

The 2026-07-19 convergence smoke re-captured the compiler-error state at 720px,
the default 420px, 320px, and the 820px narrow overlay. In every state the
absolute compiler span resolves to only `fast`; its error wave remains visible
through the overlapping Find mark, Problems reports the authored line/column,
and the last-known-good game frame remains unchanged. No new clipping,
occlusion, or density issue was found.
