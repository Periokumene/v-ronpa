...
export class StaticSvgCharacterPreviewRenderer implements CharacterPreviewArtifactRenderer {
  render(input: ResolvedCharacterPreview): string {
    ...
    const fitScale = Math.min(CHARACTER_PREVIEW_WIDTH / paddedWidth, CHARACTER_PREVIEW_HEIGHT / paddedHeight);
    ...
    return [
      `<svg xmlns="http://www.w3.org/2000/svg" width="${CHARACTER_PREVIEW_WIDTH}" height="${CHARACTER_PREVIEW_HEIGHT}" viewBox="0 0 ${CHARACTER_PREVIEW_WIDTH} ${CHARACTER_PREVIEW_HEIGHT}" role="img" aria-labelledby="preview-title">`,
      ...
      filters,
      "</defs>",
      `<rect width="${CHARACTER_PREVIEW_WIDTH}" height="${CHARACTER_PREVIEW_HEIGHT}" fill="url(#checker)"/>`,
      `<g transform="translate(${CHARACTER_PREVIEW_WIDTH / 2} ${CHARACTER_PREVIEW_HEIGHT / 2}) scale(${number(fitScale)}) translate(${number(-centerX)} ${number(-centerY)})">`,
      images,
      "</g>",
      "</svg>"
    ].join("");
